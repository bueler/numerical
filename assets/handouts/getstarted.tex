...
Finally we get to integration, which is to say \emph{Lebesgue} integration.  Here I will suppress many details.  The main preparatory ideas are of ``measureable sets'' and of a (positive) ``measure''.  Also, for these notes we restrict to integration over Euclidean space $\RR^n$ and its subsets.

Find a book on Lebesgue integration if you want to fill in the details!

The following is not really a definition because I am suppressing the precise properties satisfied by (Lebesgue) measurable subsets of $\RR^n$.  We will accept, without proof, the existence of such a well-defined and useful system of measurable sets.

\defin Consider $\RR^n$ with the usual (Euclidean) norm and metric.  The \textbf{Lebesgue measurable subsets} of $\RR^n$ include the open and closed sets and all countable unions and intersections thereof.

Intuitively, if one can describe precisely how a subset of $\RR^n$ is constructed then it will be measurable!  Even ``weird'' sets like fractals and randomly-generated subsets are measurable.  Nonetheless, there are (many) non-measurable subsets, but in their constructions these need highly-abstracted steps like the axiom of choice.
