...
\addtolength{\textwidth}{1.2in} %\setlength{\parindent}{0pt}
...
\begin{document}
\scriptsize \noindent Math 310 Numerical Analysis (Bueler) \hfill 3 October 2024
\thispagestyle{empty}

\bigskip
\Large\textbf{\centerline{Review Guide for Midterm Exam}}

\Large\textbf{\centerline{on Friday, 11 October 2024}}

\normalsize
\bigskip
The in-class Midterm Exam is \emph{closed book} and \emph{closed notes}.  No internet, calculator, or electronics are allowed.  Please bring only a writing implement.   I will aim for an exam which takes 45 minutes to do, but you will have 65 minutes if start right on time.  If you come prepared it will be easy.

\medskip
I encourage you to work with other students on this Review Guide.  Please read the relevant parts of the textbook\footnote{Greenbaum \& Chartier, \emph{Numerical Methods: \dots}, Princeton University Press 2012.} and identify the items you \emph{don't} understand as you get to them.

\newcommand{\sgpart}[1]{\bigskip\noindent\large\textbf{#1.} \normalsize}

\sgpart{Problem types}  Problems will be in these categories, \emph{with examples in italics}:
\begin{itemize}
\item apply an algorithm/method in a simple concrete case,

\emph{Do two steps of bisection on this problem.}

\item state a theorem or definition,

\emph{State Taylor's Theorem.  (I will not ask you to \emph{prove} theorems.  The two theorems you should memorize are listed below.)}

\item write a short pseudocode or \Matlab code to state an algorithm,

\emph{Write Newton's method as a \Matlab code or pseudocode.}

\item explain/show in words, and

\emph{Why is one of these methods better than another, when applied to this example?  (Write in complete sentences.)}

\item derive an algorithm.

\emph{Derive Newton's method.  Also draw a sketch which illustrates one step.}
\end{itemize}


\sgpart{Sections} See these textbook sections which we covered in lecture and homework:

\qquad 2.1--2.10, \quad 4.1--4.5, \quad 5.2--5.4, 5.7, \quad 7.1, 7.2.1--7.2.3, 7.3
...
\sgpart{Algorithms}  Please recall these algorithms from memory, or re-derive them as needed.
  \begin{itemize}
