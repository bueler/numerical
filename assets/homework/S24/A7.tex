...
\begin{document}
\scriptsize \noindent Math 617 Functional Analysis (Bueler) \hfill \emph{version 2; assigned 10 April 2024}
\normalsize\medskip
...
\prob{P34}  \ppart{a}  Suppose $\{\phi_n\}$ is an orthonormal basis of a complex Hilbert space $\cH$.  Define the map $Q\in\cL(\cH,\ell^2)$, where $\ell^2=\ell^2(\NN)$, by $(Qf)_n = \ip{\phi_n}{f}_{\cH}$.  Give a formula for $Q^*$.  Show that $Q$ is unitary.

\epart{b}  Let $T$ be an (unbounded) linear operator on $\cH$.  Suppose $\phi_n \in \cD(T)$ and $T \phi_n = \lambda_n \phi_n$, for $n\in\NN$ and $\lambda_n\in\CC$.  If $\{\phi_n\}$ is an orthonormal basis of $\cH$ then $Q$ in part \textbf{(a)} unitarily diagonalizes $T$ in the sense that
	$$Q T Q^* = M$$
defines an unbounded multiplication operator on $\ell^2$.

\medskip \noindent {\small \emph{Hints.}  For part \textbf{(a)} you may use \textbf{P33(c)}, though that is not the only way.  For part \textbf{(b)}, make sure to define the domain of $M$ \emph{and} the action of $M$ on elements of $\cD(M)$.}
